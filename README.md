...
- **ハプティクスフィードバック** — SteamVR → PC → TCP → HMD 完全振動パイプライン
- **VR睡眠モード** — 非活動検出で自動ビットレート低下+画面暗転、動きで復帰
- **HMDダッシュボード** — VR内からビットレート/codec設定を変更可能
- **コンフィグバリデーション** — 起動時に設定値を自動検証、不正値はフォールバック
